...
The range of precrastination behaviours is shown in Figure (\ref{fig:exp2_strat_summary}(b). The majority of participants clearly either pre-crastinated (117, $58.5\%$) or not (71, $35.5\%$) with only a small minority (11, $5.5\%$) following an intermediate strategy\footnote{Two participants had missing data for this section of the experiment. One due to having a broken leg, the other for an unspecified reason}. As such, we will treat pre-crastination as a two-level factor and exclude participants exhibiting intermediate behaviours. 
...
The first step of our planned analysis is to test the hypothesis that there is a difference in  visual search strategy between those participants who pre-crastinated and those who did not. As registered, we fit a Bayesian beta regression model, the results of which are displayed in Figure \ref{fig:full_exp_strategy}. There is little-to-no evidence for a difference between the two groups, ($p(\delta >0| X)  =0.50$). As such, our hypothesis 2 is somewhat redundant, but we report the $95\%$ HPDI $= [-0.0465, 0.0467$] for completeness sake.

\begin{figure}[t]
  \centering  
  \subfigure[]{\includegraphics[width=6cm]{../analysis/scratch/prior_plot.pdf}}
  \subfigure[]{\includegraphics[width=6cm]{../analysis/scratch/post_plot.pdf}}
  \caption{(a) Prior and (b) posterior distributions for the mean of the two groups of participants, and the difference between them. Our priors for the two groups are identical, hence the overlapping distributions.}
  \label{fig:full_exp_strategy}
\end{figure}

Our final registered hypothesis concerns the relationship between pre-crastination and conscientiousness. HOW SHOULD I CALCULATE THIS?


\subsubsection{Discussion}

We found no evidence for any of our registered hypothesis. 

\subsection{Exploratory Analysis}

We now investigate whether our data suggests any new hypotheses that could be of interest to other researchers. 

\subsubsection{Self Report Strategies}

When carrying out the personality measures (after having completed both the visual search and the pre-crastination parts of the experiment), we asked participants if they followed a WHAT WAS THE ACTUAL WORDING HERE consistent strategy in either or both tasks. We find that $61
\%$ and $74\%$ of participants report that they followed a strategy in the visual search and pre-crastination tasks respectively. 
